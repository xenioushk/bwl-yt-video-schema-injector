...
class FormFieldsRenderCb {
	...
	/**
	 * Get the FAQ form login required field.
	 */
	public function get_faq_logged_in_status() {
			$field_id   = 'bwl_advanced_faq_logged_in_status'; // change the id.
	...
	/**
	 * Get the FAQ form captcha status field.
	 */
	public function get_capthca_status() {
			$field_id   = 'bwl_advanced_faq_captcha_status'; // change the id.
			$field_name = $this->options_id . "[{$field_id}]";
			$value      = $this->options[ $field_id ] ?? 1; // change default value.
	...
	/**
	 * Get the new faq notification status field.
	 */
	public function get_faq_notification_status() {
		$field_id   = 'bwl_advanced_email_notification_status'; // change the id.
		$field_name = $this->options_id . "[{$field_id}]";
		$value      = $this->options[ $field_id ] ?? 1; // change default value.
	...
	public function get_notification_email() {
		...
		$field_id   = 'bwl_advanced_notification_email_id'; // change the id.
		$field_name = $this->options_id . "[{$field_id}]";
		$value      = $this->options[ $field_id ] ?? get_bloginfo( 'admin_email' ); // change default value.
		$hints      = esc_html__( 'Add a valid Email address !', 'bwl-adv-faq' );
	...
	public function get_single_faq_disable_status() {
		...
		echo $this->get_checkbox_field( $field_name, $field_id, ($value === 'on') ? true : false , '', '', ); //phpcs:ignore
	}
}
