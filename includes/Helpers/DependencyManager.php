...
class DependencyManager {
	...
	/**
	 * Plugin parent theme URL.
	 *
	 * @var string
	 */
	public static $kdesk_url;
	/**
	...
	private static function set_urls() {
		self::$kdesk_url        = "<strong><a href='https://1.envato.market/kdesk-wp' target='_blank'>Knowledgedesk WordPress Theme</a></strong>";
		...
		self::$addon_title      = '<strong>Knowledge Desk - WP Bakery Addons</strong>';
	}
	...
	private static function set_dependency_constants() {
		define( 'KDESKADDON_MIN_BKBM_VERSION', '1.5.7' );
		define( 'KDESKADDON_MIN_PHP_VERSION', '7.0' );
	}
	...
	public static function check_minimum_version_requirement_status() {
		...
		return ( version_compare( BKBM_CURRENT_PLUGIN_VERSION, KDESKADDON_MIN_BKBM_VERSION, '>=' ) );
	}
	...
	public static function notice_min_version_main_plugin() {
		...
		$message = sprintf(
            ...
            esc_html__( 'The %2$s requires %1$s %4$s or higher. You are using %3$s', 'kdesk_vc' ),
            self::$bkbm_url,
            self::$addon_title,
            BKBM_CURRENT_PLUGIN_VERSION,
            KDESKADDON_MIN_BKBM_VERSION
        );
     ...
     * @return void
     */
	public static function notice_missing_theme() {

		$message = sprintf(
						// translators: 1: Plugin name, 2: Addon title
            esc_html__( 'Please install and activate the %1$s theme to use %2$s.', 'kdesk_vc' ),
            self::$kdesk_url,
            self::$addon_title
		);
	...
	/**
     * Function to handle the missing wpb plugin notice.
     *
     * @return void
     */
	public static function notice_missing_wpb_plugin() {
		...
		$message = sprintf(
						// translators: 1: Plugin name, 2: Addon title
            esc_html__( 'Please install and activate the %1$s plugin to use %2$s.', 'kdesk_vc' ),
            self::$wpb_url,
            self::$addon_title
		);
	...
	public static function notice_missing_purchase_verification() {
		...
		$message = sprintf(
            ...
            esc_html__( 'Please Activate the %1$s to use the %2$s.', 'kdesk_vc' ),
