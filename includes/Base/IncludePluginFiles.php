...
class IncludePluginFiles {
	...
	public function register() { // phpcs:ignore
		...
		if ( ! empty( $this->frontend_files ) ) {
			foreach ( $this->frontend_files as $file ) {
				include_once KDESKADDON_FILE_PATH . "/{$file}.php";
			}
		}
	...
	private function set_frontend_files() {
		...
		$frontend_files = [
			'includes/theme_vc_helpers',
			'includes/Controllers/WPBakery/Support/KdeskNestedElements',
			'includes/Controllers/WPBakery/Support/KdeskNestedElementsItem',
		];
	...
	private function set_admin_files() {
		$admin_files = [
			'includes/Libs/Cmb/KdeskMetaBox',
		];
		return $admin_files;
	}
}
